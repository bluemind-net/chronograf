...
type CellEditorOverlayQueryTransitions = QueryTransitions & {
  editRawTextAsync: () => Promise<void>
}
type CellEditorOverlayActionsFunc = (queryID: string, ...args: any[]) => void
import {KapacitorQueryConfigActions} from 'src/types/actions'
type CellEditorOverlayActions = {
  [P in keyof KapacitorQueryConfigActions]: CellEditorOverlayActionsFunc
}
...
class CellEditorOverlay extends Component<Props, State> {
  ...
  private get queryActions(): QueryTransitions {
    const original: CellEditorOverlayQueryTransitions = {
      ...queryTransitions,
      editRawTextAsync: () => Promise.resolve(),
    }
    const mapped: CellEditorOverlayActions = _.reduce<
      CellEditorOverlayQueryTransitions,
    ...
    >(
      ...
      {
        // TODO: this param needs to match the CellEditorOverlayActions output (keys)
      }
    )
